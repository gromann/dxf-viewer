...
export class DxfScene {
  ...
  _GetUnitsScale() {
    ...
    if (insunits === undefined) {
      return 1; // Default to 1 if INSUNITS is not defined
    }
// return 1; // Default to 1 if INSUNITS is not defined
    ...
    switch (insunits) {
      ...
      case 2:
        return 30.48; // Feet to cm
      case 3:
        return 160934.4; // Miles to cm
      case 4:
        return 0.1; // Millimeters to cm
      case 5:
        return 1; // Centimeters to cm
      case 6:
        return 100; // Meters to cm
      case 7:
        return 100000; // Kilometers to cm
      case 8:
        return 2.54e-6; // Microinches to cm
      case 9:
        return 0.000254; // Mils to cm
      case 10:
        return 91.44; // Yards to cm
      case 11:
        return 1e-8; // Angstroms to cm
      case 12:
        return 1e-7; // Nanometers to cm
      case 13:
        return 1e-4; // Microns to cm
      case 14:
        return 10; // Decimeters to cm
      case 15:
        return 1000; // Decameters to cm
      case 16:
        return 10000; // Hectometers to cm
      case 17:
        return 1e11; // Gigameters to cm
      case 18:
        return 1.496e13; // Astronomical units to cm
      case 19:
        return 9.461e18; // Light years to cm
      case 20:
        return 3.086e19; // Parsecs to cm
      case 21:
        return 30.480061; // US Survey Feet to cm
      case 22:
        return 2.540005; // US Survey Inch to cm
      case 23:
        return 91.440183; // US Survey Yard to cm
      case 24:
        return 160934.721869; // US Survey Mile to cm
      default:
        return 1; // Default to 1 if INSUNITS is unknown
    }
  }
  ...
  async Build(dxf, fontFetchers) {
    ...
    for (const block of this.blocks.values()) {
      if (block.data.hasOwnProperty("entities")) {
        const blockCtx = block.DefinitionContext(this.unitsScale);
  ...
  async _FetchFonts(dxf) {
    ...
    for (const entity of dxf.entities) {
      if (IsTextEntity(entity)) {
        await ProcessEntity(entity); // Process all entities
      }
    }
    for (const block of this.blocks.values()) {
      if (block.data.hasOwnProperty("entities")) {
  ...
  _GenerateBulgeVertices(vertices, startVtx, endVtx, bulge) {
    ...
    if (numSegments > 1) {
      ...
      for (let i = 1; i < numSegments; i++) {
        ...
        const v = new Vector2(center.x + R * Math.cos(a), center.y + R * Math.sin(a));
        vertices.push(v);
      }
    }
    vertices.push(new Vector2(endVtx.x, endVtx.y));
  ...
  *_DecomposeEllipse(entity, blockCtx) {
    ...
    const vertices = [];
    const xR = Math.sqrt(
      entity.majorAxisEndPoint.x * entity.majorAxisEndPoint.x + entity.majorAxisEndPoint.y * entity.majorAxisEndPoint.y
    );
  ...
  *_DecomposePoint(entity, blockCtx) {
    ...
    if (isShaped) {
      ...
      const key = new BatchingKey(layer, POINT_SHAPE_BLOCK_NAME, BatchingKey.GeometryType.POINT_INSTANCE, color, 0);
  ...
  *_DecomposeAttribute(entity, blockCtx) {
    if (!this.textRenderer.canRender) {
      console.log("Text rendering is not supported");
      return;
    }
    ...
    try {
      // we need to ensure correct font scale and position
      const start = entity.startPoint ? this._TransformVertex(entity.startPoint) : { x: 0, y: 0 };
      const end = entity.endPoint ? this._TransformVertex(entity.endPoint) : null;
      const scaledFontSize = entity.textHeight * entity.scale * 1;
      ...
      console.error("unable to decompost attrib!", e);
    }
  ...
  *_DecomposeSolid(entity, blockCtx) {
    yield* this._DecomposeFace(entity, entity.points, blockCtx, false, this._GetEntityExtrusionTransform(entity));
  }
  ...
  *_DecomposeHatch(entity, blockCtx) {
    ...
    if (boundaryLoops.length == 0) {
      console.warn(
        "HATCH entity with empty boundary loops array " + "(perhaps some loop types are not implemented yet)"
      );
      return;
    }
    ...
    for (const seedPoint of seedPoints) {
       ...
       */
      const patTransform = pattern.offsetInLineSpace
        ? calc.GetPatternTransform({
            seedPoint,
          ...
          if (dashPatLength !== null) {
            ...
            for (let segIdx = minSegIdx; segIdx <= maxSegIdx; segIdx++) {
              ...
              for (let dashLength of line.dashes) {
                ...
                if (!isSpace) {
                  for (const seg of ClipSegment(segStartParam, segStartParam + dashLengthParam)) {
      ...
      if (loop.type & 2) {
        ...
        for (const edge of loop.edges) {
          switch (edge.type) {
            case 1:
              /* Line segment. */
              AddPoints(vertices, [new Vector2(edge.start.x, edge.start.y), new Vector2(edge.end.x, edge.end.y)]);
              break;
            case 2: {
              /* Circular arc. */
              const arcVertices = [];
            ...
            case 3: {
              ...
              const xR = Math.sqrt(
                majorAxisEndPoint.x * majorAxisEndPoint.x + majorAxisEndPoint.y * majorAxisEndPoint.y
              );
            ...
            case 4:
              ...
              for (let i = 0; i <= subdivisions; i++) {
                const pt = this._InterpolateSpline(i * step, edge.degreeOfSplineCurve, controlPoints, edge.knotValues);
  ...
  *_DecomposeSpline(entity, blockCtx = null) {
    ...
    const step = 1 / subdivisions;
    for (let i = 0; i <= subdivisions; i++) {
      const pt = this._InterpolateSpline(i * step, entity.degreeOfSplineCurve, controlPoints, entity.knotValues);
  ...
  _ProcessPoints(entity, blockCtx = null) {
    const key = new BatchingKey(entity.layer, blockCtx?.name, BatchingKey.GeometryType.POINTS, entity.color, 0);
    const batch = this._GetBatch(key);
    for (const v of entity.vertices) {
  ...
  _TransformVertex(v, blockCtx = null) {
    ...
    return { x: v.x * this.unitsScale, y: v.y * this.unitsScale };
...
class RenderBatch {
  ...
  Merge(batch, transform = null) {
    if (this.key.geometryType !== batch.key.geometryType) {
      throw new Error(
        "Rendering batch merging geometry type mismatch: " + `${this.key.geometryType} !== ${batch.key.geometryType}`
      );
    }
...
class Block {
  ...
  DefinitionContext(scale = 1) {
    return new BlockContext(this, BlockContext.Type.DEFINITION, scale);
  }
...
class BlockContext {
  ...
  TransformVertex(v, scale) {
    ...
    const result = new Vector2(v.x, v.y).applyMatrix3(this.transform.scale(effectiveScale, effectiveScale));
