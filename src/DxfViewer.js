...
export class DxfViewer {
   ...
   */
  constructor(options = null, externalRenderer = null, externalCamera = null, externalScene = null) {
    ...
    try {
      if (externalRenderer) {
        ...
        this.canvas = this.renderer.domElement;
        this.renderer.setSize(options.canvasWidth, options.canvasHeight);
      }
    } catch (e) {
...
//   /** @return {Iterable<{name:String, color:number}>} List of layer names. */
  ...
  _CanvasToSceneCoord(x, y) {
    const v = new three.Vector3((x * 2) / this.canvasWidth - 1, (-y * 2) / this.canvasHeight + 1, 1).unproject(
      this.camera
    );
    return { x: v.x, y: v.y };
  }
