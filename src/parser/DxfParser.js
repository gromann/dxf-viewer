    ...
    while (true) {
      if (curr.code === 0) {
        ...
        if (handler != null) {
          ...
          log.debug("}");
          if (handler.ForEntityName === "HATCH") {
            console.log("HATCH entity found", entity);
          }
        } else {
        ...
        // Register hatch pattern if it exists
        if (entity.type === "HATCH" && entity.patternName) {
          try {
            // const pattern = Pattern.FromHatchEntity(entity);
            let pattern = Pattern.ParsePatFile(`
                *GRASS,GRASS
